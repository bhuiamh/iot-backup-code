...
def mq_message_sent(received_dic, requestType):
    ...
    if requestType == "verify":
    	print(str(config["publish_topic"])) #+"verify")
    	publish.single(str(config["publish_topic"]), json.dumps(received_dic), hostname=mqHost, port=mqPort)
    elif requestType == "dispense":
    	print(str(config["publish_topic"])) #+"dispense")
    	publish.single(str(config["publish_topic"]), json.dumps(received_dic), hostname=mqHost, port=mqPort)
